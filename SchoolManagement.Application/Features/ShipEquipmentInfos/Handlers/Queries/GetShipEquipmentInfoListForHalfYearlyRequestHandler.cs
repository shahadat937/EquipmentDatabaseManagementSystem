...
namespace SchoolManagement.Application.Features.ShipEquipmentInfos.Handlers.Queries
{
    public class GetShipEquipmentInfoListForHalfYearlyRequestHandler :  IRequestHandler<GetShipEquipmentInfoListForHalfYearlyRequest, List<ShipEquipmentInfoDto>>
    {
        ...
        {
            IQueryable<ShipEquipmentInfo> ShipEquipmentInfos = _ShipEquipmentInfoRepository.FilterWithInclude(x => x.EquipmentCategoryId == request.EquipmentCategoryId && x.EqupmentNameId == request.EqupmentNameId && x.BaseSchoolNameId == request.ShipId, "EquipmentCategory", "EqupmentName", "StateOfEquipment", "AcquisitionMethod");
