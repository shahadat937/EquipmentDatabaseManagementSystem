...
{
    public class GetShipEquipmentInfoByCategoryIdNameIdAndStateOfEquipmentIdRequestHandler : IRequestHandler<GetShipEquipmentInfoByCategoryIdNameIdAndStateOfEquipmentIdRequest, PagedResult<ShipEquipmentInfoDto>>
    {
        private readonly ISchoolManagementRepository<ShipEquipmentInfo> _shipEquipmentRepository;
        private readonly IMapper _mapper;
        public GetShipEquipmentInfoByCategoryIdNameIdAndStateOfEquipmentIdRequestHandler(ISchoolManagementRepository<ShipEquipmentInfo> shipEquipmentRepository, IMapper mapper)
        {
            _mapper = mapper;
            _shipEquipmentRepository = shipEquipmentRepository;

        }
        public async Task<PagedResult<ShipEquipmentInfoDto>> Handle(GetShipEquipmentInfoByCategoryIdNameIdAndStateOfEquipmentIdRequest request, CancellationToken cancellationToken)
        {
            ...
            IQueryable<ShipEquipmentInfo> query = _shipEquipmentRepository.FilterWithInclude(
                x => x.EquipmentCategoryId == request.CategoryId && x.EqupmentNameId == request.EquipmentNameId &&
