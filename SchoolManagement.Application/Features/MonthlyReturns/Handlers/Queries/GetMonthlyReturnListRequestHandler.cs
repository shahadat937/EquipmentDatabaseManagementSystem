...
namespace SchoolManagement.Application.Features.MonthlyReturns.Handlers.Queries
{
    public class GetMonthlyReturnListRequestHandler : IRequestHandler<GetMonthlyReturnListRequest, PagedResult<MonthlyReturnDto>>
    {
        ...
        public async Task<PagedResult<MonthlyReturnDto>> Handle(GetMonthlyReturnListRequest request, CancellationToken cancellationToken)
        {
            ...
            IQueryable<MonthlyReturn> MonthlyReturns = _MonthlyReturnRepository.FilterWithInclude(x => (request.ShipId == 0 || x.BaseSchoolNameId == request.ShipId) && (String.IsNullOrEmpty(request.QueryParams.SearchText) || x.EqupmentName.Name.Contains(request.QueryParams.SearchText)|| x.BaseSchoolName.SchoolName.Contains(request.QueryParams.SearchText)), "EquipmentCategory", "EqupmentName", "ReportingMonth", "OperationalStatus", "ReturnType","BaseSchoolName");
