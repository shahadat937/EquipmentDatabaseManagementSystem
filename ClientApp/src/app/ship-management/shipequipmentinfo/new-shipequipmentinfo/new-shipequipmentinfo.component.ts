...
export class NewShipEquipmentInfoComponent implements OnInit {
  ...
  isFile: boolean;
  shipImage: any;
  isDamageDescriptionRequired: boolean = false;
  ...
  checkNonOplQty(event: Event) {
    if (this.ShipEquipmentInfoForm.get('nonOplQty')?.value) {
      this.isDamageDescriptionRequired = true;
    } else {
      this.isDamageDescriptionRequired = false
    }
  }
